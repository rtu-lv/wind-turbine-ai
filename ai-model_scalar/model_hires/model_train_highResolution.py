...
def train_surrogate_model(config, num_epochs, num_gpus):
    ...
    #     accel = "cpu"
    #     devs = 1
    # trainer = pl.Trainer(accelerator=accel,
    #                       devices=devs, max_epochs=num_epochs,
...
def tune_surrogate_model(num_epochs, num_samples):
    config = {
        ...
        "batch_size": tune.choice([64, 128, 256, 528])
    }
...
def load_and_cache_data(data_cache_file):
    ...
    train_dataset = AlyaDataset(os.path.join(DATA_BASE_PATH, DATA_TRAIN_SUBDIR), True, "cnnS-sc")
    test_dataset = AlyaDataset(os.path.join(DATA_BASE_PATH, DATA_TEST_SUBDIR), True, "cnnS-sc")
...
def tune_and_test():
    ...
    #     accel = "cpu"
    #     devs = 1
    # trainer = pl.Trainer(accelerator=accel, devices=devs)
